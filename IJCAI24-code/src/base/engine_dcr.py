...
class BaseEngine():
    def __init__(self, device, model, dataloader, scaler, sampler, loss_fn, lrate, optimizer, \
                 scheduler, clip_grad_value, max_epochs, patience, log_dir, logger, seed,save_path,learning_step):
        super().__init__()
        self._device = device
        self.model = model
    ...
    def train_batch(self):
        ...
        for X, label in self._dataloader['train_loader'].get_iterator():
            ...
            pred = self.model(X, label,self._trainadj)
    ...
    def evaluate(self, mode):
        ...
        if mode == 'test' or mode=='replay':
            ...
            if mode=='test':
                ...
                self._dataloader[mode + '_loader']=self._dataloader['val_loader']
                adj=self._valadj
        else:
            adj=self._valadj
        self.model.eval()
        ...
        if mode == 'replay':
            self.set_nodesnum(preds.shape[-1])
            mae = self._loss_fn(preds, labels, mask_value).item()
            mape = masked_mape(preds, labels, mask_value).item()
            rmse = masked_rmse(preds, labels, mask_value).item()
            adj_test_mae=[]
