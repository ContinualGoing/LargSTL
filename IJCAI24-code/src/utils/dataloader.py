...
def load_dataset(args, logger,adj=None):
    if args.dataset=='GBA':
        ptr = np.load('./data/gba/2020/his.npz')
    if args.dataset=='GLA':
        ptr = np.load('./data/gla/2018/his.npz')
    else:
        ptr = np.load('./data/ca/2019/his.npz')
    dataloader = {}
    idx={}
    data_len=int(ptr['data'].shape[0]/2)
    train_len=int(data_len*0.6)
    val_len=int(data_len*0.2)
    test_len=int(data_len*0.2)
    idx['train']=np.array([i for i in range(train_len)])
    idx['val']=np.array([i for i in range(train_len,train_len+val_len)])
    idx['test']=np.array([i for i in range(train_len+val_len,train_len+val_len+test_len)])
    for cat in ['train', 'val']:
        dataloader[cat + '_loader'] = DataLoader(ptr['data'][:, adj,:args.input_dim], idx[cat], \
                                                 args.seq_len, args.horizon, args.bs, logger)

    dataloader['test_loader'] = DataLoader(ptr['data'][:, adj,:args.input_dim], idx['test'], \
                                                    args.seq_len, args.horizon,args.bs, logger)
...
def load_dataset_beifen2(data_path, args, logger,adj=None):
    ...
    idx['test']=np.array([i for i in range(train_len+val_len,train_len+val_len+test_len)])
    for cat in ['train', 'val']:
        dataloader[cat + '_loader'] = DataLoader(ptr['data'][..., :args.input_dim], idx[cat], \
                                                 args.seq_len, args.horizon, args.bs, logger)
 
    #idx = np.load('/home/wbw/ICLR/LargeST-main/data/ca/2019/idx_test.npy')
    #ptr = np.load('/home/wbw/ICLR/LargeST-main/data/ca/2019/his.npz')
    dataloader['test_loader'] = DataLoader(ptr['data'][..., :args.input_dim], idx['test'], \
                                                 args.seq_len, args.horizon, args.bs, logger)

    scaler = StandardScaler(mean=ptr['mean'], std=ptr['std'])
    return dataloader, scaler

def load_dataset_beifen(data_path, args, logger):
    ptr = np.load(os.path.join(data_path, args.years, 'his.npz'))
    logger.info('Data shape: ' + str(ptr['data'].shape))
    
    dataloader = {}
    idx={}
    idx['train_loader']=None
    ...
    idx = np.load('/home/wbw/ICLR/LargeST-main/data/ca/2019/idx_test.npy')
    ptr = np.load('/home/wbw/ICLR/LargeST-main/data/ca/2019/his.npz')
    dataloader['test_loader'] = DataLoader(ptr['data'][..., :args.input_dim], idx, \
                                                 args.seq_len, args.horizon, args.bs, logger)
...
def get_dataset_info(dataset):
    base_dir = '/home/wbw/ICLR/LargeST-main/data/'
    d = {
